...
namespace SimPatient
{
	public class MedicationDose
	{
		...
		public static void refreshRemainingMedicationDosePool(long pat_id)
		{
			if (MySqlHelper.connect() == false) return;

			DBConnection dbCon = MySqlHelper.dbCon;
			ArrayList response = dbCon.selectQuery(string.Format("CALL get_remaining_patient_doses({0})", pat_id));
